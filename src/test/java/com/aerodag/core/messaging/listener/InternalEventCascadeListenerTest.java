...
class InternalEventCascadeListenerTest {
  ...
  void setUp() {
    listener =
        new InternalEventCascadeListener(
            nodeRepository, nodeQueuePublisher, planRepository, sseNotificationService);
  ...
  void onNodeCompleted_pendingNodeWithUnmetDeps_notPublished() {
    ...
    Node pending =
        nodeWithStatus(pendingId, "node-3", NodeStatus.PENDING, List.of("node-1", "node-2"));
  ...
  void onNodeCompleted_allNodesCompleted_marksPlanCompleted() {
    ...
    Plan plan =
        Plan.builder().id(planId).globalObjective("test").status(PlanStatus.IN_PROGRESS).build();
  ...
  private Node nodeWithStatus(
      UUID id, String nodeId, NodeStatus status, List<String> dependencies) {
    return Node.builder()
        .id(id)
        .nodeId(nodeId)
        .status(status)
