...
public class RedisNodeWorkerListener {
  ...
  private void processNode(UUID nodeId) {
    ...
    try {
      ...
      ChatClient.ChatClientRequestSpec request =
          chatClient.prompt().system(systemPrompt).user(node.getInstruction());
      ...
      eventPublisher.publishEvent(
          new NodeFailedEvent(node.getPlan().getId(), node.getId(), e.getMessage()));
      return;
    }
  ...
  private String buildSystemPrompt(Node node) {
    ...
    List<Node> depNodes =
        nodeRepository.findByPlanIdAndNodeIdIn(node.getPlan().getId(), depNodeIds);
    ...
    return EXECUTOR_SYSTEM_PROMPT
        + "\nHere is the data from previous steps: "
        + contextString
        + ".";
  }
}
