...
public class PlannerService {
  ...
  public PlanResponse generateAndSaveDag(String userObjective) {
    String rawJson = chatClient.prompt().system(SYSTEM_PROMPT).user(userObjective).call().content();
    ...
    List<Node> nodes = dagResponse.nodes().stream().map(nr -> mapToNode(nr, plan)).toList();
    ...
    List<NodeResponse> nodeResponses =
        nodes.stream().map(n -> new NodeResponse(n.getInstruction())).toList();
